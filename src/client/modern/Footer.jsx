...
function SocialIconItem({ link, children, handleActiveElement }) {
  ...
  return (
    <a
      ...
      onMouseOver={() => handleActiveElement(ref.current)}
      onMouseOut={() => ref.current.classList.remove("active")}
      onFocus={() => handleActiveElement(ref.current)}
      onBlur={() => ref.current.classList.remove("active")}
    >
      {children}
    </a>
  );
